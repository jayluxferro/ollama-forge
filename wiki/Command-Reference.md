...
| **vlm** | Vision Language Model commands: generate, chat, serve, video-generate, convert, quantize, finetune (Apple Silicon via mlx-vlm >=0.4.4; TurboQuant KV, VisionFeatureCache, ORPO) |
...
| VLM generate with TurboQuant KV | `vlm generate --model <model> --prompt "..." --image photo.jpg --kv-bits 3.5` |
...
| VLM convert HF → MLX | `vlm convert --hf-path <repo_id> [--quantize --q-bits 4 --q-mode mxfp4]` |
...
- **vlm** has subcommands: `generate`, `chat`, `chat-ui`, `serve`, `video-generate`, `convert`, `quantize`, `finetune`. Requires Apple Silicon (mlx-vlm >=0.4.4). Supports TurboQuant KV cache (`--kv-bits 3.5`), VisionFeatureCache (`--vision-cache-size`), video input, ORPO training, and mxfp4/nvfp4/mxfp8 quantization. See [VLM](VLM) for details.
...
- **Ports:** Ollama 11434, serve 11434, abliterate serve 11435, abliterate proxy 11436, turboquant serve 8811, vlm serve 11434 (defaults; override with `--port`).
