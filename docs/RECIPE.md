...
Build an Ollama model from a **recipe file** (YAML or JSON) with a single command: `ollama-forge build <recipe.yaml>`. To validate without building: `ollama-forge build <recipe.yaml> --validate-only`.
...
- **quantize** — Quantize the GGUF before create (only with **gguf**; e.g. `Q4_K_M`). Mirrors `convert --quantize`; requires llama.cpp `quantize` on PATH.
- **gguf_file** — Specific .gguf filename (only with **hf_repo**; use when repo has multiple).
- **quant** — Preferred quantization when the HF repo has multiple GGUF files (only with **hf_repo**; e.g. `Q4_K_M`, `Q8_0`). Mirrors `fetch --quant`.
- **revision** — HF repo revision (only with **hf_repo**; default: main).

To validate without building: `ollama-forge validate-recipe recipe.yaml` or `ollama-forge build recipe.yaml --validate-only`.
...
```bash
ollama-forge build recipe.yaml
ollama-forge build recipe.yaml --validate-only   # check recipe only
ollama-forge build recipe.yaml --out-modelfile Modelfile.generated
```

The recipe must contain exactly one of **base**, **gguf**, or **hf_repo**. See [wiki/Recipes.md](../wiki/Recipes.md) for more examples.
